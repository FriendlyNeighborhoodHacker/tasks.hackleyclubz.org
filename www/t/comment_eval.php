<?php
// Adds a comment via the token-authenticated email flow (POST from t/index.php).
// The token authenticates its user for exactly this task; no session login is
// created and the token passes through back to t/index.php.
...
try {
    TaskManagement::addComment($ctx, $auth['task_id'], (string)($_POST['comment'] ?? ''));
    ActivityLog::log($ctx, 'task.comment_via_token', ['task_id' => $auth['task_id']]);
    $_SESSION['t_success'] = 'Comment posted.';
} catch (Throwable $e) {
