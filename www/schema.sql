...
-- instead of the global SMTP_* constants in config.local.php — e.g. so a
-- group's emails come from its own Gmail address (with an app password).
-- from_email falls back to smtp_username; from_name falls back to the group
-- name. Defaults live in config; a row exists only when a group customizes.
